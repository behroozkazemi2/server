var providerList = function () {
    ...
    var listGrid = function () {
        $(document).on('click', '#gridTabs a', function () {
            ...
            if (providerGrid && checkChangeStatus && changedStatus == selectedTabGrid) {
                $('#providerList' + selectedTabGrid).dataTable().fnDestroy();
                checkChangeStatus = false;
            }
            $('input[name="status"]').val(selectedTabGrid);
            providerListGrid = $('#providerList' + selectedTabGrid).DataTable({
                "processing": true,
                "serverSide": true,
                "searching": false,
                "retrieve": true,
    ...
    var searchProvider = function () {
        $(document).on('click', '#search_btn', function () {
            ...
            $('#providerList' + selectedTabGrid).dataTable().fnDestroy();
            $('#providerList' + selectedTabGrid).DataTable({
                "processing": true,
                "searching": false,
                "serverSide": true,
                "pageLength": 10,
    ...
    loadDataTable = function(response){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var row = {
                ...
                numbers: numbersStr,
                action: '<div class="btn-group"><a href="/admin/provider/products/' + data[i].id + '" type="button" class="btn btn-sm btn-primary">محصولات</a>' +
                '<a href="/admin/provider/edit?id=' + data[i].id + '" type="button" class="btn btn-sm btn-secondary text-light">ویرایش</a>' +
                        action + '<a class="btn btn-sm btn-danger text-light"  onclick="deleteProduct(' + data[i].id + ')" >' +
                ' حذف </a></div>'
            };
            all.push(row);
    ...
    reloadList = function () {
        ...
        $('#providerList' + selectedTabGrid + '_wrapper').removeClass('hidden');
        $('#providerSearchList' + selectedTabGrid + '_wrapper').addClass('hidden');
